...
import { addMake } from "../../../../../../services/vehicleManamentService";

interface AddMakesProps {
  open: boolean;
  onClose: () => void;
  onSuccess?: () => void;
}

export default function AddMakes({ open, onClose, onSuccess }: AddMakesProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!logoFile) {
      setError("Please upload a logo (JPG, PNG or WebP).");
      return;
    }
    const allowed = ["image/jpeg", "image/jpg", "image/png", "image/webp"];
    if (!allowed.includes(logoFile.type)) {
      setError("Logo must be JPG, JPEG, PNG or WebP.");
      return;
    }
    setLoading(true);
    setError(null);
    try {
      const res = await addMake(name, logoFile);
      if (res.status) {
        handleClose();
        onSuccess?.();
      } else {
        setError(res.message || "Failed to add manufacturer.");
      }
    } catch (e: unknown) {
      const message = e instanceof Error ? e.message : "Failed to add manufacturer.";
      setError(message);
    } finally {
      setLoading(false);
    }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm">
      ...
      >
        ...
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-xl font-bold text-gray-800">Add Make</h2>
          <button
            type="button"
            onClick={handleClose}
