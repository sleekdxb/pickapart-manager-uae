...
import { updateModel, type MakeModel } from "../../../../../../services/vehicleManamentService";
...
export default function EditModel({ open, onClose, model, onSuccess }: EditModelProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await updateModel(model.make_id, model.model_id, name);
      if (res.status) {
        handleClose();
        onSuccess?.();
      } else {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm">
      ...
      >
        ...
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-xl font-bold text-gray-800">Edit Model</h2>
          <button
            type="button"
            onClick={handleClose}
